...
const RequestTimer = new Cron.CronJob(
	'00 10 9-23/3 * * 1-6', () => { // (Hopefully) On the 10th minuteEvery 3 hours from 9-23h except on sundays
		const d = new Date(Date.now());
		...
		Notification.findAll({
			...
			listNotification.forEach((element) => {
				...
				User.findOne({
					...
					bot.beginDialog(userData.address, 'secondMissionConclusion:/');
				}).catch((errUser) => {
					console.log(`Coundn't find User => ${errUser}`);
				});
			});
