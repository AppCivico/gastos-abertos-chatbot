...
library.dialog('/', [
	(session) => {
		...
		'dando-lhe permissão para mandar mensagems diretas para os usuários em nome do grupo!' +
		'\nInsira o nome do perfil, escolha na lista e confirme. A pessoa não se tornará um administrador.');
	...
	(session, results) => {
		...
		builder.Prompts.choice(
			session, `Deseja adicionar ${userName} no grupo ${userGroup}? Ele receberá direito de mandar mensagens diretas para o usuário.`,
			[Confirm, Cancel],
			{
				listStyle: builder.ListStyle.button,
				retryPrompt: retryPrompts.addAdmin,
